...
- **Единый ручной контур:** **`POST /api/a2a/sessions`**, затем **`/next`** + poll **`/async`**; типичный UX — **два удара**: текст направления работы, потом **`choice`** по списку роутера (Агент / …). См. `AGENTS.md` → *Unified manual path* и *Router dialog (two beats)*. Таблица «почему оборвали итерацию» и чеклист драйвера: `AGENTS.md` → *Why iteration stops*; `docs/OPERATOR-CURL.md` → *Driver checklist (anti-stop)*. **Одна линия «стек → API → промпты `prompts-to-agent-mode` → наблюдение → улучшение»:** [`prompts-to-agent-mode/ONE-PIPELINE.md`](../prompts-to-agent-mode/ONE-PIPELINE.md); корневой мастер-промпт полного прогона: [`START-FULL-SPECTRUM.md`](../START-FULL-SPECTRUM.md).
