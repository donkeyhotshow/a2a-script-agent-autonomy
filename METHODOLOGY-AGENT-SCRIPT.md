...
ЗАДАЧА считается незавершённой ЕСЛИ:
  ...
  - И файл НЕ имеет соответствующей записи в /tasks/completed/ или /tasks/failed/
  - ИЛИ файл не обновлялся более 3 циклов
```
...
1. Проверить, что все задачи, приведшие к активации режима 2:
   - Имеют записи в `/tasks/completed/` или `/tasks/failed/`
   - С timestamps позже времени активации режима 2
...
|---------|----------|----------|------------------|
| `cycle_time` | Время выполнения одного цикла | Логи AI Director | < 60 сек |
| `task_success_rate` | Процент успешно выполненных задач | `/tasks/completed/` | > 95% |
...
### 7.2 Политика хранения и архивации

> **Статус**: Рекомендуется к реализации

**Правила ротации логов:**

| Тип данных | Максимальный возраст | Действие при превышении |
|------------|----------------------|--------------------------|
| Runtime логи | 24 часа | Сжать и переместить в archive/old/ |
| Session логи | 7 дней | Удалить (или переместить в long-term storage) |
| Debug логи | 3 дня | Автоматически удалить |
| Task результаты | 30 дней | Переместить в archive/completed/ |

**Реализация:**
- Добавить задачу очистки в режим обслуживания (или отдельную сессию cleanup)
- Проверять размер папки `/logs/archive/` перед каждым циклом
- Если размер > X MB → принудительно активировать режим обслуживания
