...
**Status:** done (2026-04-03) — `rag.test.js` exercises indexer/searcher/chunk/BM25/hybrid/reranker/query-understanding; invalid TS `!` non-null assertions removed for Vitest parse.
...
- [x] All placeholder tests removed and real assertions added.
- [x] `npm test` passes for `a2a-client/packages/rag/tests/rag.test.js` without reliance on network/integration fixtures.
- [x] Tracked in root `DEV_STATE` RAG line on next prune.
