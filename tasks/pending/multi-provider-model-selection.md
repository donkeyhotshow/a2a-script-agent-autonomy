...
1. **Model selector**
   ...
   - Settings page for default model preference

2. **Files to modify**
   - Web UI session creation form
   - Settings panel

## Acceptance Criteria

- [ ] `/api/tags` returns both Z.AI and Ollama models when both providers enabled
- [ ] Each model entry has `provider` field indicating backend
- [ ] Requests with explicit `model` route to correct provider
- [ ] a2a-server propagates model selection to ai-integration
- [ ] ADR documenting multi-provider model contract
