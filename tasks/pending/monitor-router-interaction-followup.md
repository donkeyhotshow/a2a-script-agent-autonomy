...
**Status:** closed (2026-04-03) — historical session `sess_*` 404 is expected after storage prune; **mitigation in code:** monitor auto-submits router choice (order **`agent` → `task-decomposition` → `dialog` → first**) and re-sends task text on idle task forms; daemon completion path retries the same gate before “no result”. **Storage:** `DELETE /api/a2a/sessions/:id` returns **409** when flat **or project** storage shows in-flight promise (`getProjectModeInflightPromise` for `.a2a/session-steps` + snapshot); manual archive scripts still bypass API. **Client API:** single `GET /sessions/:id` handler so `?includeContext=1` is not shadowed by an earlier branch (flat message timeline + prod guard).  
