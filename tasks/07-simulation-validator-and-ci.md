...
3. **CI integration**
   ...
   - Wire it into:
     ...
     - CI pipeline (GitHub Actions, etc.) so that PRs touching `simulations/` or `new-request-flow/json-schemas` must pass validation.
4. **Documentation**
   - Extend `new-request-flow/SCHEMAS.md` or create a small doc (e.g. `new-request-flow/SIMULATION-VALIDATION.md`) that explains:
     - What is validated.
