...
- `new-request-flow/` — architecture, protocol, schemas, simulation format.
...
- Simulation format doc: `new-request-flow/SIMULATION-FORMAT.md`
- Data schemas doc: `new-request-flow/SCHEMAS.md`
- Protocol doc: `new-request-flow/PROTOCOL.md`
- Session flow: `new-request-flow/SESSION-FLOW.md`
- Action map: `new-request-flow/ACTION-MAP.md`
- Current issues / roadmap / implementation status:
  - `new-request-flow/CURRENT-ISSUES.md`
  - `new-request-flow/ROADMAP.md`
  - `new-request-flow/IMPLEMENTATION-STATUS.md`

## Work to perform

1. **Inventory & alignment**
   - Scan `docs/`, `new-request-flow/`, root `AGENTS.md`, and any `NEW-*` docs.
   - For each document, decide: keep, merge into canonical docs, or delete as obsolete.
   - Ensure `new-request-flow/` is the single source of truth for:
...
3. **Canonicalization**
   - Ensure all references in docs use the new:
     ...
     - Asynchronous `promiseId` flow for LLM (where applicable), consistent with:
       - `new-request-flow/SIMULATION-LLM-PROXY.md`
...
4. **Cleanup**
   ...
   - Update cross-links so that new readers land on `new-request-flow/` docs and `simulations/SCHEMA.md`.

## Acceptance criteria

- No conflicting or duplicated protocol descriptions; `new-request-flow/` + `simulations/SCHEMA.md` are the clear canon.
