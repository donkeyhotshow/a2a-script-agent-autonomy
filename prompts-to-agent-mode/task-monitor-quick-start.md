...
## Sources
- [`monitor-and-process-tasks.js`](../monitor-and-process-tasks.js)
...
- `hooks/`
- [`COMPLETION-REPORT.md`](../COMPLETION-REPORT.md) (stub index → MONITOR-QUICK-START + git)

## Agent prompt (copy)
Write or refresh [`MONITOR-QUICK-START.md`](../MONITOR-QUICK-START.md) so it mirrors `monitor-and-process-tasks.js` + `tests/monitor-tasks/*` and the **static** Vitest suite. If `COMPLETION-REPORT.md` does not exist, create it at the repository root; if it already exists, update it. The reference should cover:
...
8. **Test Suite** – document `npm run test:monitor` (repo root) and **19** static tests over the entry file + `tests/monitor-tasks/*.js`.
...
## Completion
- [x] Done — canonical doc is [`MONITOR-QUICK-START.md`](../MONITOR-QUICK-START.md); [`COMPLETION-REPORT.md`](../COMPLETION-REPORT.md) stub added; static suite = **19** (`npm run test:monitor`).
