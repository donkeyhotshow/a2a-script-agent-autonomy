...
## Agent prompt (copy)
Write or refresh [`MONITOR-QUICK-START.md`](../MONITOR-QUICK-START.md) so it mirrors `monitor-and-process-tasks.js` + `tests/monitor-tasks/*` and the **static** Vitest suite. The reference should cover:
...
8. **Test Suite + monitor defaults** – document `npm run test:monitor` (repo root) and static tests over the entry file + `tests/monitor-tasks/*.js`; note default **`TASK_MONITOR_STRICT_AGENT_COMPLETION=0`** (poll-only, no auto **`/next`**) in [`monitor-and-process-tasks.js`](../monitor-and-process-tasks.js); opt-in strict nudges + env vars in [`MONITOR-QUICK-START.md`](../MONITOR-QUICK-START.md) troubleshooting.
