...
**Два типа действий в первом ответе:** (1) **actions** — первоочередно; шаги захардкожены, сервер сам переключает шаг. (2) **ai-actions** — второстепенно (диалог с LLM); отображается список доступных шагов, следующий шаг определяется из ответа LLM; возможен отдельный запрос на каждый шаг.
