...
**Recent (direct-tests):** `start-all.bat` ends with `tests/direct-tests/run-post-start-all.ps1` (full stack checks + Vitest + node + PS1 flows; `A2A_POST_START_SKIP_HEAVY=1` skips long LLM steps). All runners and `e2e-dialog-test.js` / `gray-room-test.js` live under `tests/direct-tests/`; nested `.ps1` wrappers use `Join-Path $PSScriptRoot '..\..\..'` for repo root; `npm run test:direct-tests` = Vitest (`lib/a2a-schema-guards.mjs` + `router-choice-transition.test.mjs`, live invoke suite skipped unless `GET {A2A_SERVER_URL}/health` OK). CLI repro: `node tests/direct-tests/router-choice-transition-run.mjs`. **`e2e-dialog-test.js`:** sticky-router cases include `routerAgentNoLoop*` and **`routerDialogNoLoop`** / **`routerDialogNoLoopTaskShorthand`** (after **dialog** choice); `routerWrongBeatMessage`. **`replay-session-from-disk.js`:** `--assert-no-sticky-router` exits non-zero if a router `choice` / choice-shorthand `/next` leaves `task`/`router` + `form.choices`. **Router→mode transition:** shared `router-choice-transition-core.mjs` — verifies router→mode transition (dialog/agent/task-decomposition choices); server fix in `action-request-processor.ts` pre-applies choice before delegating to dialog processor.
