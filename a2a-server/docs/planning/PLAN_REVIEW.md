...
After reviewing `a2a-server/docs/planning`, several issues no longer match current `a2a-server` code. To scale planning, each issue should map to a real module and simulations/LLM logic should be checked against the current stack (`ActionService`, `dialogRequestProcessor`, `determineRequestType`). Canonical simulations are under repo root [`simulations/`](../../../simulations/) — see [`REFERENCE.md`](REFERENCE.md).

## Findings

1. **Issue 1 (router)** — The doc mentions a missing `loadFromDirectory` call, but the registry is initialized via `a2a-server/src/actions/action-service.ts` (see `initialize`). Update the issue text to explain where to find `ActionRegistry` logs and how to reload `fix-vue-imports` / `agent-coder` goldens.
...
2. Update Issues 1 and 3 so the simulation test loop includes `npx tsx a2a-server/scripts/run-simulation.ts simulations/<sim>/<step>` (path to a step folder) and log checks (`ActionRegistry`); Issue 2 should verify `context.history` is written only via `buildDialogProcessResultFromContext`.
