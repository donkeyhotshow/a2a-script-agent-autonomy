...
import {VALID_EXECUTE_KEYS} from '../../packages/actions/src/action-validator.ts';
...
/** Repo root: scripts/sim-lint → scripts → a2a-server → repo */
const REPO_ROOT = join(__dirname, '..', '..', '..');
export const SIMULATIONS_DIR = join(REPO_ROOT, 'simulations');
export const INTEGRATION_SIMULATIONS_DIR = join(REPO_ROOT, 'tests', 'integration', 'simulations');
...
/** True if `dir` is a golden sim root: flat bundle or at least one numeric step with `request.json`. */
function isSimulationRoot(dir: string): boolean {
    if (!existsSync(dir)) return false;
    if (REQUIRED_FILES.every((f) => existsSync(join(dir, f)))) {
        return true;
    }
    if (existsSync(join(dir, 'request.json')) && existsSync(join(dir, 'response.json'))) {
        return true;
    }
    try {
        for (const e of readdirSync(dir, {withFileTypes: true})) {
            if (!e.isDirectory()) continue;
            if (/^\d+$/.test(e.name) && existsSync(join(dir, e.name, 'request.json'))) {
                return true;
            }
            if (/^\d+-sub-\d+$/.test(e.name)) {
                const sub = join(dir, e.name);
                if (existsSync(join(sub, 'request.json')) || existsSync(join(sub, 'response.json'))) {
                    return true;
                }
            }
        }
...
function collectSimulationsUnder(SIM_ROOT: string, namePrefix: string): {path: string; name: string}[] {
    const simulations: {path: string; name: string}[] = [];
    if (!existsSync(SIM_ROOT)) return simulations;

    const entries = readdirSync(SIM_ROOT, {withFileTypes: true});

    for (const entry of entries) {
        if (entry.isDirectory()) {
            const subDir = join(SIM_ROOT, entry.name);
            ...
            if (existsSync(subDir)) {
                try {
                    ...
                    for (const subEntry of subEntries) {
                        ...
                        const candidate = join(subDir, subEntry.name);
                        if (!isSimulationRoot(candidate)) continue;
                        const nm = `${entry.name}/${subEntry.name}`;
                        simulations.push({
                            path: candidate,
                            name: namePrefix ? `${namePrefix}/${nm}` : nm,
                        });
                    }

                    const mainSimPath = join(SIM_ROOT, entry.name);
                    if (isSimulationRoot(mainSimPath)) {
                        simulations.push({
                            path: mainSimPath,
...
export function getAllSimulations(): {path: string; name: string}[] {
    return [
        ...collectSimulationsUnder(SIMULATIONS_DIR, ''),
        ...collectSimulationsUnder(INTEGRATION_SIMULATIONS_DIR, 'integration'),
    ];
}
