...
/** Repo `docs/PROTOCOL/json-schemas` (three levels up: sim-validate → scripts → a2a-server → repo). */
export const SCHEMAS_DIR = join(__dirname, '..', '..', '..', 'docs', 'PROTOCOL', 'json-schemas');
...
export function validateSimulation(simPath: string, simName: string, opts: ValidateOptions): SimulationValidationResult {
    ...
    // Валидация каждого файла (interrupt substep dirs: только request/response — см. SCHEMA.md / sim-lint runners)
    const interruptSubstepRe = /^\d+-sub-\d+$/;
    const isInterruptSubstep = interruptSubstepRe.test(basename(simPath));
    const requiredFiles = isInterruptSubstep
        ? ['request.json', 'response.json']
        : ['request.json', 'response.json', 'client.json', 'received.json'];
    const optionalFiles = isInterruptSubstep
        ? ['client.json', 'received.json', 'server-transforms-request.json', 'server-transforms-response.json']
        : ['server-transforms-request.json', 'server-transforms-response.json'];
    ...
    for (const filename of allFiles) {
        ...
        if (
            isInterruptSubstep &&
            (filename === 'client.json' || filename === 'received.json') &&
            !existsSync(filePath)
        ) {
            continue;
        }
