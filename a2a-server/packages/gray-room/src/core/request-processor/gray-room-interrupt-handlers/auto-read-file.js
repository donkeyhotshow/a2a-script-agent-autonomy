...
export class HandleAutoReadFile extends BaseGrayRoomHandler {
    async handleInterrupt(interrupt, ctx, promiseId, aiHubUrl, model, trace) {
        const fp = (interrupt.data?.filePath || interrupt.data?.path);
        if (!fp) {
        ...
        if (out.success && out.content !== undefined) {
            ...
            const prevFiles = innerCtx.files ?? {};
...
export async function handleAutoReadFile(interrupt, ctx, promiseId, aiHubUrl, model, trace) {
    const handler = new HandleAutoReadFile();
    return handler.handle(interrupt, ctx, promiseId, aiHubUrl, model, trace);
}
//# sourceMappingURL=auto-read-file.js.map