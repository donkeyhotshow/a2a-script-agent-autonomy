...
} from "../../../../server/src/request-processor/validators/transform-execute-validator.js";
import {
  resolveExecution,
...
} from "../../../../server/src/request-processor/llm-model-resolver.js";
...
} from "../request-processor/gray-room-trigger.js";
...
} from "../request-processor/gray-room-utils.js";

// Import interrupt handlers
import { handleCompressHistory } from "../request-processor/gray-room-interrupt-handlers/compress-history.js";
import { handleThinking } from "../request-processor/gray-room-interrupt-handlers/thinking.js";
import { handleAutoReadFile } from "../request-processor/gray-room-interrupt-handlers/auto-read-file.js";
...
import { globalIntentGate } from "../../../../server/src/intent-gate.js";
...
export class GrayRoomOrchestrator {
  ...
  async runLoop(
    ...
    logger.info("[GrayRoom] gray_room.loop.enter", {
      evt: "gray_room.loop.enter",
      trace_id: traceId,
    ...
    return new Promise<ProcessResult>((resolve) => {
      const processTick = async () => {
        ...
        if (executeCall) {
          ...
          if (mcpTool) {
            ...
            try {
              ...
              const fix = await bugFixer.fix(
                JSON.stringify(executeCall[actionName]),
                e.message,
              );
              if (fix.fixed) {
        ...
        if (!interrupt) {
          ...
          const primaryTurnComplete =
            this.isResponseTransformCompleted(rawOutput);

          // -- INTENT DRIFT CHECK (ADR-0050) --
          if (globalIntentGate.getTurnCount() >= 5) {
            const executeKey = Object.keys(result.execute || {}).find(
              (k) => k !== "noop" && k !== "message" && k !== "form",
            );
            const currentPlan =
              executeKey || (workingCtx["task"] as string) || "";

            const driftCheck = await globalIntentGate.checkDrift(
              currentPlan,
              workingCtx,
            );
          ...
          if (primaryTurnComplete) {
            logger.info(
              "[GrayRoom] Primary turn result.completed=true. Entering SIEGE_REVIEW",
            );
            const reviewResult =
              await globalRoleRegistry.executeSyndicateReview(workingCtx);
            if (reviewResult.passed) {
              logger.info(
                "[GrayRoom] SIEGE_REVIEW passed. Completing session.",
              );
            } else {
        ...
        if (
          typeof interrupt.maxTurns === "number" &&
        ...
        logger.info("[GrayRoom] Iteration start", {
          turn,
          state: currentState,
          trace_id: resolveA2aTraceId(workingCtx, promiseId),
          promise_id: promiseId,
        });
        ...
        let lastOutcome =
          (workingCtx["result"] as Record<string, any>)?.["outcome"] || "noop";

        // -- OVERRIDE OUTCOME FOR REVIEW/DEBATE (ADR-0038) --
        if (
          currentState === OrchestratorState.REVIEWING &&
        ...
        if (
          currentState === OrchestratorState.DEBATING &&
        ...
        const { nextState, artifact } = this.kernel.transition(
          currentState,
          lastOutcome,
          workingCtx,
        );
        const lastAction =
          (workingCtx["execute"] as Record<string, any>)?.["write-file"] ||
        ...
        if (isUIChange) {
          try {
            ...
            const devUrl =
              process.env.A2A_PREVIEW_URL || "http://localhost:5173"; // Default Vite port
            const screenshotPath = `storage/screenshots/turn-${turn}.png`;
            await globalVisionTester.captureScreenshot(devUrl, screenshotPath);
            const visionResult = await globalVisionTester.performVisualQA(
              screenshotPath,
              (workingCtx["task"] as string) || "UI matching manifesto",
            );
            ...
            logger.error("[GrayRoom] Vision QA error", { error: String(err) });
          }
        }
        // --------------------------------------
        ...
        trace.push({
          ...
          continueLoop,
          note:
            interrupt.reason === "auto_rag_page"
        ...
        if (!continueLoop) {
          const normalizedExecute = rawOutput.execute as
        ...
        const files =
          (requestTransformResult.files as Record<string, string>) || {};
        ...
        const messages = prepareLlmMessages(
          files,
          (workingCtx["system_instruction_override"] as string | undefined) ||
            undefined,
        );

        const subHeader = `${promiseId}-intr-${interruptBudget}`;
        const llmModel = resolveGrayRoomLlmModelFromContext(
          workingCtx,
          this.model,
        );
        const chatInit = await initAiHubChatPromise(
          ...
          undefined,
          resolveA2aTraceId(workingCtx, promiseId),
        );
        if (!chatInit.ok) {
  ...
  private async applyInterrupt(
    ...
    switch (reason) {
      case "compress_history": {
        return await handleCompressHistory(
          interrupt,
          ctx,
          promiseId,
          this.aiHubUrl,
      ...
      case "auto_read_file": {
        return await handleAutoReadFile(
          interrupt,
          ctx,
          promiseId,
          this.aiHubUrl,
      ...
      case "auto_rag_page": {
        return await handleAutoRagPage(
          interrupt,
          ctx,
          promiseId,
          this.aiHubUrl,
  ...
  private warnOnInvalidExecute(
    ...
    if (shouldEnforceTransformStrictMode()) {
      throw new Error(
        `Gray room transform contract violation (${source}): ${issues.map((i) => i.code).join(", ")}`,
      );
    }
