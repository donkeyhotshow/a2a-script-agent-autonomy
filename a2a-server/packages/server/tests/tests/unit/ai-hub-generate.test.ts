...
import {fetchAiHubGenerateText} from '../../src/utils/ai-hub-generate.js';
...
describe('fetchAiHubGenerateText', () => {
    ...
    it('throws on non-ok', async () => {
        vi.stubGlobal(
            'fetch',
            vi.fn(async () => new Response('err', {status: 502, statusText: 'Bad'}))
        );
        await expect(
            fetchAiHubGenerateText('http://hub', {model: 'm', prompt: 'p', stream: false}, 5000)
        ).rejects.toThrow(/502/);
    });
