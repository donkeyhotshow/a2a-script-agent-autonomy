...
${historyJson}`;
      ...
      try {
        const init = await initAiHubChatPromise(
          aiHubUrl,
          `${promiseId}-swing-${index}`,
          {
            model: sidecarModel,
        ...
        if (init.ok) {
          const compressedRaw =
            init.inlineResponseBody ??
            (await pollReadyThenFetch(aiHubUrl, init.llmPromiseId));
          ...
          if (compressedStr) {
            ...
            if (Array.isArray(parsed)) {
              const lengthRatio =
                JSON.stringify(parsed).length / historyJson.length;
