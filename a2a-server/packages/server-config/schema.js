...
const intSchema = (min, max, defaultValue) => z.coerce.number().int().min(min).max(max).default(defaultValue);
/** Port number schema (1-65535) */
const portSchema = (defaultPort) => intSchema(1, 65535, defaultPort);
/** URL string schema */
const urlSchema = (defaultUrl) => defaultUrl ? z.string().url().default(defaultUrl) : z.string().url();
/** String with minimum length */
const minStringSchema = (min) => z.string().min(min);
/** Optional string with minimum length */
const optionalMinStringSchema = (min) => z.string().min(min).optional();
// ===========================================
