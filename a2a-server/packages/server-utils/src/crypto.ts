...
function getEncryptionKey(): Buffer {
  ...
  if (!key) {
    if (process.env["NODE_ENV"] === "production" || process.env["A2A_REQUIRE_SECRETS"] === "1") {
      throw new Error(
        "ENCRYPTION_KEY or JWT_SECRET must be set in production or when A2A_REQUIRE_SECRETS=1",
      );
    }
...
export async function hashPassword(password: string): Promise<string> {
  const { hash } = (await (import("bcrypt") as unknown as Promise<{
    hash: (p: string, r: number) => Promise<string>;
  }>)) as { hash: (p: string, r: number) => Promise<string> };
  return hash(password, 10);
}

export async function verifyPassword(password: string, hash: string): Promise<boolean> {
  const { compare } = (await (import("bcrypt") as unknown as Promise<{
    compare: (p: string, h: string) => Promise<boolean>;
  }>)) as { compare: (p: string, h: string) => Promise<boolean> };
