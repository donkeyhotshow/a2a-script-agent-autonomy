/**
 * LLM hub promise pipeline (init + poll + fetch) without coupling to `@a2a/server-request`.
 * Daemon wraps {@link pollReadyThenFetch} to patch A2A context on each poll tick when needed.
 */

import { A2A_TRACE_HEADER } from './trace-constants.js';
import { logger } from './logger.js';
import { tryParseJsonFromLlmText } from './strip-markdown-json-fence.js';
import { resolveAiHubBaseUrl } from './ai-hub-url.js';

/** Standard headers for AI hub JSON `POST` bodies. */
...
export async function fetchLlmResponse(base: string, llmPromiseId: string): Promise<string | null> {
    ...
    try {
        ...
        logger.warn('[llm-hub-poll] fetchLlmResponse formatted failed, trying raw', {
            llmPromiseId,
            error: String(e),
        });
        try {
            ...
            logger.warn('[llm-hub-poll] fetchLlmResponse raw also failed', {
                llmPromiseId,
                error: String(e2),
            });
            return null;
    ...
    try {
        ...
        logger.warn('[llm-hub-poll] fetchLlmResponse body read failed', {
            llmPromiseId,
            error: String(e),
        });
        return null;
...
export type PollReadyThenFetchOpts = {
    /**
    ...
    /** Optional hook each poll iteration (e.g. A2A request context patch). */
...
export async function initAiHubChatPromise(
    ...
    const tid = typeof a2aTraceId === 'string' ? a2aTraceId.trim() : '';
    if (tid) {
...
export async function pollReadyThenFetch(
    base: string,
    llmPromiseId: string,
    opts?: PollReadyThenFetchOpts
): Promise<string | null> {
    const normalizedBase = resolveAiHubBaseUrl(base);
    const pollIntervalMs = readEnvMs(
