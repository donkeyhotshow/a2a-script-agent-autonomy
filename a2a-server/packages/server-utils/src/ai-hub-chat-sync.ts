...
export { AI_HUB_JSON_HEADERS, type AiHubChatRequestBody };
...
export async function fetchAiHubChatJson(
    ...
    let initRes: Awaited<ReturnType<typeof initAiHubChatPromise>>;
    try {
        initRes = await initAiHubChatPromise(base, serverPromiseId, body, signal);
    } catch (e) {
    ...
    if (initRes.inlineResponseBody !== undefined && initRes.inlineResponseBody !== '') {
        ...
        try {
            polled = await pollReadyThenFetch(base, initRes.llmPromiseId, {
