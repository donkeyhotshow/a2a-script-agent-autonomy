...
export function analyzeTaskDetail(taskText: string): TaskAnalysisResult {
  ...
  for (const pattern of FILE_PATH_PATTERNS) {
    ...
    if (matches) filePaths.push(...matches);
  }
...
function determineTaskType(words: string[]): TaskAnalysisResult["taskType"] {
  for (const word of words) {
    ...
    if (TASK_TYPE_KEYWORDS.modification.some((k) => lower.includes(k))) return "modification";
    if (TASK_TYPE_KEYWORDS.creation.some((k) => lower.includes(k))) return "creation";
    if (TASK_TYPE_KEYWORDS.analysis.some((k) => lower.includes(k))) return "analysis";
  }
  return "unknown";
}
