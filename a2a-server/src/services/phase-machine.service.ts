...
// Экспорт singleton для использования в request-processor
let currentMachine: PhaseMachine | null = null;

export function getPhaseMachine(context: Record<string, unknown> = {}): PhaseMachine {
    if (!currentMachine) {
        currentMachine = new PhaseMachine(context);
    }
    return currentMachine;
}

export function resetPhaseMachine(newContext: Record<string, unknown> = {}): PhaseMachine {
    currentMachine = new PhaseMachine(newContext);
    return currentMachine;
}
