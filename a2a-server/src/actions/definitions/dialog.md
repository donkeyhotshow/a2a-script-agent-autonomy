# Action: Dialog (AI Conversation with Tool Management)
...
```typescript
export default async function collectContext(input: { parsed_intent: { intent: string; needsTools: boolean; toolTypes: string[] } }) {
  const { intent, needsTools, toolTypes } = input.parsed_intent;
  const context: { conversation_history: unknown[]; available_tools: string[] } = {
...
export default async function llmResponse(input: { 
  parsed_intent: { intent: string; needsTools: boolean; toolTypes: string[]; original: string }; 
  dialog_context: { available_tools: string[] };
  conversation_history: Array<{ role: string; content: string }>;
}) {
  const { needsTools, toolTypes, original } = input.parsed_intent;
...
${needsTools ? 'Пользователь запросил использование тулзов: ' + toolTypes.join(', ') : ''}`;
  ...
  const messages = [
    ...
    ...input.conversation_history.slice(-10),
    { role: 'user', content: original }
  ];
  
  // Вызов LLM через внешний хаб
  const { callLLM } = await import('../../services/llm-hub.service.js');
  ...
  return {
    ai_response: {
      ...
      requires_continue: response.requires_user_confirmation ?? false
    }
  };
}
```
...
export default async function executeTools(input: { 
  ai_response: { tools_to_use: string[] };
  project_context: { query?: string; target?: string; question?: string; entity?: string };
}) {
  ...
  for (const tool of toolsToUse) {
    ...
    switch (tool) {
      case 'file-search':
        // Используем file-scanner из a2a-client
        result = { status: 'placeholder', message: 'File search requires client context' };
        break;
      case 'code-analysis':
        result = { status: 'placeholder', message: 'Code analysis requires neurons' };
        break;
      case 'rag':
        result = { status: 'placeholder', message: 'RAG requires client RAG service' };
        break;
      case 'knowledge-graph':
        result = { status: 'placeholder', message: 'Knowledge graph requires client graph' };
        break;
      default:
...
export default async function formatResponse(input: { 
  ai_response: { message: string; requires_continue: boolean };
