 ...
 */
export function isGraphComplete(graph, taskContext) {
    const missing = [];
    ...
    if (taskContext?.taskText?.toLowerCase().includes('crud')) {
        if (!hasController)
            missing.push('Missing Controller for CRUD operation');
        if (!hasModel)
            missing.push('Missing Model for CRUD operation');
    }
 ...
 */
export function filterEntitiesByType(graph, type) {
    return graph.entities.filter(e => e.type === type);
}
/**
 * Find entity by path
 */
export function findEntityByPath(graph, path) {
    return graph.entities.find(e => e.path === path);
}
/**
 * Find entity by name
 */
export function findEntityByName(graph, name) {
    return graph.entities.filter(e => e.name.toLowerCase() === name.toLowerCase());
}
/**
 * Get relations for entity
 */
export function getRelationsForEntity(graph, entityId) {
    return {
 ...
 */
export function parseGraphFromContext(context) {
    ...
    const entities = Array.isArray(graph['entities'])
        ? graph['entities'].filter((e) => typeof e === 'object' && e !== null && 'id' in e && 'type' in e && 'path' in e)
        : [];
    const relations = Array.isArray(graph['relations'])
        ? graph['relations'].filter((r) => typeof r === 'object' && r !== null && 'id' in r && 'type' in r && 'fromPath' in r)
        : [];
    return { entities, relations };
}
/**
 ...
 */
export function mergeRecognizedIntoGraph(existing, recognized) {
