...
REM Root node_modules is required for repo scripts (e.g. npm run monitor:once, dotenv).
if not exist "node_modules" (
    echo [A2A] Root node_modules missing — running npm install...
    call npm install
    if errorlevel 1 (
        echo [FATAL] npm install failed at repo root.
        endlocal
        exit /b 1
    )
)

REM If stack ports already listen: non-interactive mode auto-stops via kill-all.bat.
set "STACK_BUSY=0"
for %%p in (3000 3001 5173) do (
    netstat -ano 2>nul | findstr ":%%p " | findstr "LISTENING" >nul 2>&1
    if not errorlevel 1 set "STACK_BUSY=1"
)
if "%STACK_BUSY%"=="1" (
    if /i "%A2A_START_NONINTERACTIVE%"=="1" (
        echo [A2A] Ports busy — A2A_START_NONINTERACTIVE=1, calling kill-all.bat...
        if exist "%~dp0kill-all.bat" ( call "%~dp0kill-all.bat" ) else ( echo [WARN] kill-all.bat missing — free ports manually. )
        powershell -Command "Start-Sleep -Seconds 2"
    ) else (
        echo [WARN] One or more stack ports ^(3000/3001/5173^) already in use. Run kill-all.bat or set A2A_START_NONINTERACTIVE=1 for auto cleanup.
    )
)
...
REM Best-effort stale log trim (skip if file locked)
echo [A2A] Clearing stale service logs ^(best-effort^)...
powershell -NoProfile -Command "foreach($rel in @('a2a-client\logs\client-api.log','a2a-server\logs\server.log','a2a-client\logs\web-ui.log')){ $fp=Join-Path (Get-Location) $rel; if(Test-Path $fp){ try{ [IO.File]::WriteAllText($fp,'') } catch { Write-Host ('[A2A] Skip locked log: ' + $rel) } } }" 2>nul
...
REM ==========================================
echo.
echo [Step 1/8] Cleaning environment (freeing ports)...
REM Deterministic port-based cleanup; do not depend on external kill scripts.
call :wait_port_free %SERVER_PORT% 10
if errorlevel 1 goto startup_failed
call :wait_port_free %CLIENT_API_PORT% 10
if errorlevel 1 goto startup_failed
call :wait_port_free %WEB_PORT% 10
if errorlevel 1 goto startup_failed
REM Hub must be recyclable for smoke/simulation; use same aggressiveness as other stack ports.
call :wait_port_free %PROXY_PORT% 10
powershell -Command "Start-Sleep -Seconds 2"
...
call scripts\scripts\start-ai-integration.bat
...
call scripts\scripts\start-promise-queue-daemon.bat
if errorlevel 1 set EXIT_CODE=1
...
call scripts\scripts\start-a2a-server.bat
if errorlevel 1 set EXIT_CODE=1
:server_done
...
call scripts\scripts\start-client-api.bat
if errorlevel 1 set EXIT_CODE=1
:client_api_done
...
call scripts\scripts\start-web-ui.bat
if errorlevel 1 set EXIT_CODE=1
:web_ui_done
