    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      while ((m = IMPORT_RE.exec(lines[i])) !== null) {
        ...
        if (spec.startsWith('.')) {
          ...
          if (!resolveExists(resolved)) broken.push({ file, line: i + 1, specifier: spec });
        } else if (Object.keys(paths).length) {
          const resolved = resolveAlias(spec, baseDir, paths);
          if (resolved && !resolveExists(resolved)) broken.push({ file, line: i + 1, specifier: spec });
        }
      }
    }
  }
...
function readViteAliases(viteConfigPath: string, _rootDir: string): Record<string, string[]> {
  try {
    ...
    while ((m = re.exec(block)) !== null) {
      ...
      const k = key === '@' ? '@/*' : key.endsWith('/') ? key + '*' : key + '/*';
...
function resolveAlias(spec: string, baseDir: string, paths: Record<string, string[]>): string | null {
