...
export function resolveImports(
  ...
  for (const imp of brokenImports) {
    ...
    if (imp.specifier.startsWith('.')) {
      ...
      if (absPath && !existsSync(absPath)) absPath = tryResolveExists(absPath) ?? absPath;
      if (!absPath) {
        const base = imp.specifier.split(/[/\\]/).pop() ?? '';
        const candidates = findAllCandidates(baseDir, base);
        if (candidates.length === 1) absPath = candidates[0];
...
function loadAliases(tsconfigPath: string, viteConfigPath?: string): Record<string, string[]> {
...
function readViteAliases(viteConfigPath: string, baseDir: string): Record<string, string[]> {
  try {
