...
2. **Единый «язык» данных** — скриптовая часть функций системы должна **говорить на том же языке**, что и агентская часть, чтобы **можно было отображать данные** (один контракт / одна форма представления).
...
| S10 | `agent/` без `description.md`; дрейф роутера vs `shared/router-static-choices.json` | **partial** — добавлен `simulations/sync/agent/description.md`; выравнивание текста роутера с shared — по желанию ([`tasks/sync-documentation-and-router-drift.md`](tasks/sync-documentation-and-router-drift.md)) |
