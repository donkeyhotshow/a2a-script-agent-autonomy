...
function printSuccess(): void {
    ...
    console.log(`  Server Port:      ${config.ports.serverPort}`);
    console.log(`  Client API Port:  ${config.ports.clientApiPort}`);
    ...
    if (isSafe) {
        ...
        if (isJson) {
            const output: ValidationResult = result.success
                ? {valid: true, timestamp: new Date().toISOString(), config: result.config as Record<string, unknown>}
                : {valid: false, timestamp: new Date().toISOString(), errors: result.errors!};
            outputJson(output);
        } else {
            printHeader();
            if (result.success) {
        ...
        try {
            ...
            if (isJson) {
                outputJson({valid: false, timestamp: new Date().toISOString(), errors});
            } else {
                printHeader();
                printErrors(errors);
            }
