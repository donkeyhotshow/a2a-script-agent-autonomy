...
const STATE_COLORS: Partial<Record<OrchestratorState, string>> = {
  ...
  REFLECT:    'bg-violet-900 text-violet-300 border-violet-700',
  SYNTHESIZE: 'bg-fuchsia-900 text-fuchsia-300 border-fuchsia-700',
  ENRICH:     'bg-pink-900 text-pink-300 border-pink-700',
  PLAN:       'bg-rose-900 text-rose-300 border-rose-700',
  EXECUTE:    'bg-orange-900 text-orange-300 border-orange-700',
};

const DEFAULT_STATE_COLOR = 'bg-zinc-800 text-zinc-300 border-zinc-700';
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-800 bg-zinc-950">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <span className={`px-2 py-0.5 rounded border text-xs font-mono shrink-0 ${STATE_COLORS[session.state] ?? DEFAULT_STATE_COLOR}`}>
          {session.state}
        </span>
