...
### Agent mode and repository tools
...
Router determines request routing logic based on keywords. Version 2.0 uses **keyword-based routing** via LLM transform:

1. **On receiving keyword request** recognizes user actions or choices
2. **On selection** redirects to corresponding choices (dialog, agent, task-decomposition)

**Usage Examples:**
- `dialog`: for regular user dialogue
- `agent`: for agent work (code, analysis, generation)
- `task-decomposition`: for task decomposition into subtasks

**Fallback Choices:**
```json
[
  { "id": "dialog", "label": "AI dialog with interface", "description": "..." },
  { "id": "agent", "label": "Agent (code development)", "description": "..." },
  { "id": "task-decomposition", "label": "Task decomposition", "description": "..." }
]
```

See [`a2a-server/docs/Router.md`](a2a-server/docs/Router.md) for details.
