...
- [ ] Валидация `promiseId` параметра (формат: `req_*`)
- [ ] Валидация структуры context
- [ ] Валидация codeBlocks (максимальный размер, типы файлов)
...
**Предложения:**
- [ ] Проверка что запрос принадлежит клиенту
- [ ] Проверка доступа к context проекта
...
**Предложения:**
- [ ] Пагинация списка запросов
- [ ] Фильтрация по статусу, дате, clientId
- [ ] Batch операции (bulk cancel, bulk delete)
- [ ] Приоритизация запросов (priority update)
...
**Предложения:**
- [ ] Кастомные ошибки для каждого типа (NotFound, Validation, Timeout)
- [ ] Unified error response format
- [ ] Логирование с контекстом (correlation ID)
- [ ] Retry policy для клиента
...
- [ ] Ограничение на создание запросов в минуту
- [ ] Квота на размер queue для клиента
...
- [ ] Метрики по статусам (completed/failed/pending)
