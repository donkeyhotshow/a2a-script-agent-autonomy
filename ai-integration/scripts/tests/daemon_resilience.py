#!/usr/bin/env python3
"""
Daemon resilience check for Ollama disconnect/reconnect flow.
"""
...
def run_daemon_resilience_test(proxy_url: str = DEFAULT_PROXY_URL, wait_seconds: float = 2.5) -> bool:
    ...
    print("[2/5] Stop Ollama")
    stop_result = _post_json(f"{base}/ollama/stop")
    ...
    print("[4/5] Start Ollama")
    start_result = _get_json(f"{base}/ollama/start", timeout=60.0)
    ...
    if not daemon_up.get("running"):
        ...
        return False
    if not health_up.get("ollama_available"):
