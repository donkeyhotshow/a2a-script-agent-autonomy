...
function extractModelRelations(content: string): ModelRelation[] {
  ...
  // ОПТИМІЗОВАНО: Використовуємо один прохід замість multiple regex викликів
  // Паттерн для всіх типів відносин
  const relationPattern = /(?:public|protected)\s+function\s+(\w+)\s*\(\)\s*\{[\s\S]*?return\s+\$this->(belongsTo|hasMany|hasOne|belongsToMany|morphTo|morphMany)\(([^)]+)\)/g;
  ...
  while ((match = relationPattern.exec(content)) !== null) {
    if (match[1] && match[2]) {
      relations.push({
        name: match[1],
        type: match[2] as 'belongsTo' | 'hasMany' | 'hasOne' | 'belongsToMany' | 'morphTo' | 'morphMany',
        related: match[3]?.trim().replace(/['"]/g, ''),
      });
    }
  }
  
  // Додатково: морфічні зв'язки без параметрів (morphTo)
  const morphToPattern = /(?:public|protected)\s+function\s+(\w+)\s*\(\)\s*\{[\s\S]*?return\s+\$this->morphTo\(\)/g;
  while ((match = morphToPattern.exec(content)) !== null) {
    const matchGroup = match[1];
    if (matchGroup) {
      // Перевіряємо чи не дублюємо
      if (!relations.some(r => r.name === matchGroup)) {
        relations.push({
          name: matchGroup,
          type: 'morphTo',
        });
