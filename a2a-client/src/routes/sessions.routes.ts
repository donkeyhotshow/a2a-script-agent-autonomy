 ...
 */
router.post('/', authenticate, validateBody(createSessionSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.get('/', authenticate, validateQuery(listSessionsSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const sessions = await sessionService.getByProjectId(projectId as string, req.client!.id, {
...
router.post('/:sessionId/root-context', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const dbSession = await sessionService.getById(sessionId, req.client!.id);
    if (!dbSession) {
...
router.get('/:sessionId', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const session = await sessionService.getById(sessionId, req.client!.id);
 ...
 * Update session
 */
router.patch('/:sessionId', authenticate, validateBody(updateSessionSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { sessionId } = req.params;
    const { title, status } = req.body;

    const session = await sessionService.update(sessionId, req.client!.id, {
      title,
      status,
    });
...
router.delete('/:sessionId', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const deleted = await sessionService.delete(sessionId, req.client!.id);
 ...
 */
router.post('/:sessionId/messages', authenticate, validateBody(createMessageSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { sessionId } = req.params;
