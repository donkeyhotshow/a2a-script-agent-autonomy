 ...
 * ВИПРАВЛЕНО: Додано Zod валідацію
 */
router.post('/', authenticate, validateBody(createRequestSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 * Get request status
 * ВИПРАВЛЕНО: Додано валідацію params
 */
router.get('/:promiseId/status', authenticate, validateParams(requestIdParamSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { promiseId } = req.params;

    const status = await requestService.getStatus(promiseId, req.client!.id);
...
router.get('/:promiseId/result', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await requestService.getResult(promiseId, req.client!.id);
...
router.delete('/queue/pending', authenticate, async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const { cancelledCount } = await requestService.cancelAllPending(req.client!.id);
...
router.delete('/:promiseId', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const cancelled = await requestService.cancel(promiseId, req.client!.id);
