...
/**
 * Feature modules - these would be dynamically imported based on config
 */
interface FeatureModules {
  rag?: () => Promise<void>;
  embedding?: () => Promise<void>;
...
class A2AClientApplication {
  ...
  private initializeFeatures(): void {
    ...
    if (this.config.features.rag) {
      ...
      // this.features.rag = await import('./features/rag').then(m => m.initialize);
    }

    if (this.config.features.embedding) {
      console.log('Embedding feature enabled');
      // this.features.embedding = await import('./features/embedding').then(m => m.initialize);
    }

    if (this.config.features.execution) {
      console.log('Execution feature enabled');
      // this.features.execution = await import('./features/execution').then(m => m.initialize);
    }

    if (this.config.features.web) {
      console.log('Web feature enabled');
      // this.features.web = await import('./features/web').then(m => m.initialize);
    }

    if (this.config.features.storage) {
      console.log('Storage feature enabled');
      // this.features.storage = await import('./features/storage').then(m => m.initialize);
    }
  }
