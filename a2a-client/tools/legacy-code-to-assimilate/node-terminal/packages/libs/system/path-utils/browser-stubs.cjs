 ...
 */
export function initializeFsModules() {
  const fs = require('fs');
  ...
  if (isBrowser) {
    ...
    try {
      ...
      // Fallback для случаев, когда require не работает
      console.warn('[PATH-UTILS] Fallback to browser mode due to require error:', error.message);
