...
// Заглушки для зависимостей, которые будут мокированы в тестах или перенесены позже
const { getCurrentDir } = require('../workdir-utils/index.cjs'); // Обновлено для корректного разрешения модуля
...
class SessionManager {
	static getHistoryRoot() {
		return path.join(process.cwd(), 'history');
	}

	static getSessionsDir() {
		return path.join(this.getHistoryRoot(), 'sessions');
	}
	...
	static getCurrentSessionFile() {
		return path.join(this.getHistoryRoot(), 'CURRENT');
	}
	...
	static listSessions() {
		...
		SessionManager.ensureDir(sessionsDir);
		const sessions = [];
		...
		try {
			...
			// console.error('listSessions: Error listing sessions', error); // Отладка
		}

		sessions.sort((a, b) => String(b.modified).localeCompare(String(a.modified)));
		// console.log('listSessions: returning sessions', sessions.map(s => s.session_id)); // Отладка
		return sessions;
	}
...
class HistoryRecordManager {
	...
	static buildOrderedRecord(sid, safeRecord) {
		...
		const reason = typeof safeRecord.reason === 'string' ? safeRecord.reason : '';
		...
		if (typeof safeRecord.duration === 'string') ordered.duration = safeRecord.duration;
		if (typeof safeRecord.stdout === 'string' && safeRecord.stdout.length) ordered.stdout = safeRecord.stdout;
		if (typeof safeRecord.stderr === 'string' && safeRecord.stderr.length) ordered.stderr = safeRecord.stderr;
	...
	static safeTruncate(text) {
		const limit = configManager.getOutputCharLimit();
	...
	static async persistHistoryRecord(record) {
		try {
			...
			console.error('[ERROR] persistHistoryRecord failed:', error);
		}
	}
...
;
export { persistHistoryRecord,
	pushMemoryHistory,
	listSessions: SessionManager.listSessions, // Экспортируем статический метод
	loadSessionRecords: SessionManager.loadSessionRecords, // Экспортируем статический метод
	getCurrentSessionId,
	setCurrentSessionId,
