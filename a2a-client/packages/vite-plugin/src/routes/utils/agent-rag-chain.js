...
import { runClientReadFile, runClientListDirectory, runClientFileExists, runClientWriteFile, runClientGrepSearch, } from './chain-tools-fs.js';
...
async function runClientToolForExecute(cwd, projectPath, toolKey, payload) {
    ...
    const safeProject = typeof projectPath === 'string' && projectPath.trim() ? projectPath : safeCwd;
    ...
    switch (toolKey) {
        ...
        case 'run-script': {
            ...
            if (typeof p.command === 'string' && p.command.trim()) {
                ...
                const inlinePayload = typeof p.timeout === 'number'
                    ? p
                    : { ...p, timeout: 600000 };
 ...
 */
export async function chainSyncInvokesForAgentTools({ cwd, sessionId, a2aServerUrl, startStepNum, serverResponse, mergedContext, messages, maxDepth, }) {
    ...
    let projectPath = process.env.A2A_RAG_PROJECT_PATH || process.env.A2A_PROJECT_PATH || getProjectPathForSessions(cwd);
