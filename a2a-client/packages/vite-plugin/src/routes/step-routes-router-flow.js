...
export { routerFormHasChoices, normalizeRouterStepSubmit, buildSubmitResult, validateSubmitResult, } from '@a2a-client/shared/router-submit.js';
...
export async function handleRouterFlow({ cwd, path, req, res, url, storageMode = 'storage' }) {
    ...
    if (req.method === 'GET' && latestMatch) {
        ...
        const { projectPath, cleanup, notFound } = beginProjectStepContext(cwd, sessionId, url, storageMode);
        if (notFound) {
            res.writeHead(404).end(JSON.stringify({ error: 'Session not found (unknown project)' }));
            return true;
        }
