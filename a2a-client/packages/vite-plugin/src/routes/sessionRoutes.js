...
function locateProjectModeSession(cwd, sessionId, q = {}) {
    ...
    if (session)
        return { session, projectPath: primary };
    ...
    if (!found)
        return { session: null, projectPath: primary };
    ...
    return async (req, res, next) => {
        ...
        if (req.method === 'GET' && p === '/sessions') {
            const sessions = storageMode === 'project'
                ...
                : listNewSessions(cwd);
            try {
                const existingSessionIds = new Set((Array.isArray(sessions) ? sessions : [])
        ...
        if (sequenceMatch && (req.method === 'GET' || req.method === 'PUT')) {
            ...
            if (storageMode === 'project') {
                res.writeHead(501).setHeader('Content-Type', 'application/json').end(JSON.stringify({ error: 'sequence.tson is only supported in default (flat) session storage' }));
                return;
            }
            ...
            if (req.method === 'PUT') {
                ...
                req.on('end', () => {
                    try {
                        ...
                        let existing = null;
                        if (storageMode === 'project') {
                            const qPid = typeof d.projectId === 'string' && d.projectId.trim()
                                ? d.projectId.trim()
                                : null;
                            const qRoot = typeof d.projectRoot === 'string' && d.projectRoot.trim()
                                ? d.projectRoot.trim()
                                : null;
                            try {
                    ...
                    if (s) {
                        ...
                        if (inflight?.promiseId) {
                            res.writeHead(409, { 'Content-Type': 'application/json' }).end(JSON.stringify({
                                error: 'Session has in-flight async work; wait for completion before delete',
