...
function compactPromiseForStorageIfNeeded(merged) {
    ...
    const hint = typeof merged.message === 'string'
...
function runViteClientPromisePoll({ cwd, sessionId, promiseId, currentStep, requestUrl, res, includePromiseIdInBody, projectPath, }) {
    ...
    const xhrReq = http.request(reqOptions, (xhrRes) => {
        ...
        xhrRes.on('end', async () => {
            ...
            try {
                ...
                stepHandlers.saveServerPromise(cwd, sessionId, currentStep, compactPromiseForStorageIfNeeded(updatedPromise));
                ...
                if (isPromisePollComplete(updatedPromise)) {
                    ...
                    const mergedForChain = (stepRecord && stepRecord.context) ||
                    ...
                    const nextPid = (wrapped && wrapped.data && wrapped.data.promiseId) ||
                ...
                const webExecute = errWire || !wireServerBody.execute
                    ? null
                    : buildExecuteProjection(wireServerBody.execute, {
                        context: pollCtx && typeof pollCtx === 'object' && pollCtx !== null
                            ? pollCtx
                            : undefined,
                    });
...
export async function handleAsyncFlow({ cwd, url, path, req, res, storageMode = 'storage' }) {
    ...
    if (req.method === 'GET' && asyncMatch) {
        ...
        const { projectPath, cleanup, notFound } = beginProjectStepContextAsync(cwd, sessionId, url, storageMode);
        if (notFound) {
