    ...
    async _checkStatus(promiseId) {
        ...
        for (let retry = 0; retry <= this.maxRetries; retry++) {
            try {
                ...
                const timeoutId = useAbort && controller
                ...
                const pollData = await pollRes.tson();
...
export async function pollA2ARequestResult(promiseId, options = {}) {
    const { baseUrl = defaultBaseUrl(), maxPolls = Number.POSITIVE_INFINITY, intervalMs = 1000, headers = {}, fetchImpl = globalThis.fetch, onProgress, } = options;
