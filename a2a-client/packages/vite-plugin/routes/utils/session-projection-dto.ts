...
import { buildExecuteProjection, buildWebExecute } from './execute-projection-dto.ts';
import { collectSessionMessagesFlat } from './message-timeline.ts';
import { deriveSessionStage } from './session-stage-machine.ts';
...
export function getProjectModeInflightPromise(cwd, sessionId, projectPath, sessionObj) {
    if (projectPath && typeof projectPath === 'string') {
        ...
        try {
            ...
            if (hit?.promiseId) return { promiseId: hit.promiseId };
        } finally {
