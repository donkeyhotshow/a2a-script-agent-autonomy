        ...
        if (sequenceMatch && (req.method === 'GET' || req.method === 'PUT')) {
            ...
            if (storageMode === 'project') {
                res.writeHead(501).setHeader('Content-Type', 'application/json').end(
                    JSON.stringify({ error: 'sequence.json is only supported in default (flat) session storage' })
                );
                return;
            }
            const seqPath = path.join(getNewSessionDir(cwd, sessionId), 'sequence.json');
            if (req.method === 'GET') {
