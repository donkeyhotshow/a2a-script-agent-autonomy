...
export class AsyncApiClient {
  ...
  async createRequest(data: Record<string, unknown>): Promise<{ promiseId: string; requestId?: string; status?: string }> {
    const res = await this.request('POST', '/requests', data);
    return (res as { data?: { promiseId: string; requestId?: string; status?: string } }).data ?? (res as { promiseId: string; requestId?: string; status?: string });
  }
  ...
  async runSessionWithRAG(
    opts: {
      projectPath: string;
      task?: string;
      rag?: { searcher: { search: (q: string, opts: { limit: number }) => Promise<Array<{ chunk: { filePath: string } }>> } };
      maxIterations?: number;
    },
    callbacks: { onIteration?: (n: number, result: unknown) => void; onStatus?: PollCallbacks['onStatus']; onComplete?: (result: unknown) => void } = {}
  ): Promise<unknown> {
