...
import {sanitizeInvokeBodyForA2aUpstream} from '../../lib/a2a-invoke-builders.js';
...
import {
    persistSyncThenRagChain,
    setStepNum,
    toWebClientSessionPayload,
    validateRequestToServer,
        ...
        if (body.task) {
            try {
                ...
                if (upstream.ok && serverResponse) {
                    ...
                    console.error('[SESSIONS API] Server call failed:', upstream.status, serverResponse);
                }
            } catch (serverError) {
                console.error('[SESSIONS API] Failed to call server for task:', serverError);
            }
        }
