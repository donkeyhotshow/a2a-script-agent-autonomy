...
export function buildMinimalNextAck(step: number, promiseId: string | null, traceId?: string | null) {
    return {
...
export async function invokeAndPersistContinuation(params: {
    ...
    res: Response;
}): Promise<{ ackStep: number; promiseId: string | null; traceId?: string } | null> {
    ...
    try {
        ...
        const ctx = upstreamBody['context'];
        let traceHeader: string | undefined;
        if (ctx && typeof ctx === 'object' && !Array.isArray(ctx)) {
            const t = (ctx as Record<string, unknown>)[A2A_TRACE_CONTEXT_KEY];
        ...
        const upstream = await serverFetch(
            ...
            '/api/v1/invoke',
            upstreamBody,
            traceHeader ? { [A2A_TRACE_HEADER]: traceHeader } : undefined
        );
        ...
        return { ackStep, promiseId, traceId: traceHeader };
    } catch (serverError) {
        res.status(503).json({
            success: false,
            error: {
