        ...
        try {
            ...
            if (upstream.ok) {
                ...
                if (promiseId) {
                    ...
                    // Timeout - return session in current state
                    await saveSession(project, updatedSession);
                    res.status(201).json({
                        session: toSessionDetail(updatedSession),
                        serverError: 'Request timeout',
                    });
                    return;
                } else {
                    // Sync response - update session directly with payload data
                    const completedSession = await updateSessionWithServerResponse(project, updatedSession, payload?.data || payload);

                    if (hasFormChoices({ execute: (payload?.data || payload)?.execute })) {
                        completedSession.status = 'READY';
                    }
                    ...
                    const syncResponse = {
                        session: toSessionDetail(completedSession),
                        serverResponse: { data: payload?.data || payload },
...
expressApp.all(['/api/requests*', '/api/v1/requests*'], async (req, res) => {
    ...
    if (pathName.includes('/status') && payload) {
        try {
            const promiseId = extractSessionIdFromPath(pathName); // This extracts promiseId from URL
            if (promiseId) {
                const projects = await loadProjects();
                ...
                // Find session that has this promiseId as lastPromiseId
                for (const p of projects) {
                    const sessionsDir = getSessionDir(p);
                    if (fs.existsSync(sessionsDir)) {
                        ...
                        for (const sessionFile of sessionFiles) {
                            try {
                                ...
                                if (sessionData.lastPromiseId === promiseId) {
                                    session = sessionData;
                                    project = p;
                ...
                if (project && session && sessionId) {
                    ...
                    await saveSession(project, updatedSession);
                    const responsePromiseId: string | undefined = payload?.data?.promiseId || payload?.promiseId;
                    broadcastProgress(sessionId, {
...
expressApp.patch(['/api/sessions/:sessionId', '/api/v1/sessions/:sessionId'], async (req, res) => {
    ...
    // Only allow updating specific safe fields (prevent overwriting id, projectId, createdAt, etc.)
    const allowedUpdates: Partial<Session> = {};
    const allowedFields = ['title', 'task', 'status', 'selectedAction', 'lastPromiseId', 'messages', 'version', 'execution'];
