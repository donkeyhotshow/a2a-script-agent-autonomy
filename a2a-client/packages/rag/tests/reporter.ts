/**
 * @fileoverview Test reporting system for RAG testing infrastructure
 ...
 * const markdown = generateMarkdownReport(report, { includeRecommendations: true });
 ...
 * @example
 * ```typescript
 * const json = generateJSONReport(report, { pretty: true });
 * await fs.writeFile('report.json', JSON.stringify(json, null, 2));
 * ```
 */
 ...
 * @example
 * ```typescript
 * const html = generateHTMLReport(report, { includeCharts: true });
 * await fs.writeFile('report.html', html);
 * ```
 */
 ...
 * @example
 * ```typescript
 * await saveReport(report, './reports/test-report.md', 'markdown');
 * await saveReport(report, './reports/test-report.html', 'html');
 * await saveReport(report, './reports/test-report.json', 'json');
 * ```
 */
 ...
 * @example
 * ```typescript
 * const paths = await generateAllReports(report, './reports', 'test-run-123');
 * console.log(paths.markdown); // ./reports/test-run-123.md
 * ```
 */
