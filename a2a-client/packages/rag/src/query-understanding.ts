...
export class QueryUnderstandingEngine {
    ...
    private _detectIntents(_query: string, query: string, _terms: string[]): Array<{
        type: string;
        confidence: number
    }> {
        const intents: Array<{ type: string; confidence: number }> = [];
        if (this.patterns.exact_name.some((p) => p.test(query))) intents.push({
            type: INTENT_TYPES.EXACT_NAME,
            confidence: 0.95
        });
        if (this.patterns.code_pattern.some((p) => p.test(query))) intents.push({
            type: INTENT_TYPES.CODE_PATTERN,
            confidence: 0.9
        });
        if (this.patterns.dependency.some((p) => p.test(query))) intents.push({
            type: INTENT_TYPES.DEPENDENCY,
            confidence: 0.85
        });
        if (this.patterns.file_path.some((p) => p.test(query))) intents.push({
            type: INTENT_TYPES.FILE_PATH,
            confidence: 0.9
        });
        if (this.patterns.symbol.some((p) => p.test(query))) intents.push({
            type: INTENT_TYPES.SYMBOL,
            confidence: 0.85
        });
        if (this.patterns.documentation.some((p) => p.test(query))) intents.push({
            type: INTENT_TYPES.DOCUMENTATION,
            confidence: 0.8
        });
    ...
    private _selectPrimaryIntent(intents: Array<{ type: string; confidence: number }>): {
        type: string;
        confidence: number
    } {
        ...
        if (sorted.length > 1 && sorted[0].confidence - sorted[1].confidence < 0.2) return {
            type: INTENT_TYPES.MIXED,
            confidence: 0.7
        };
        return sorted[0];
    }
