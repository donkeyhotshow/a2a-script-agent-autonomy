/**
 * Embedding client (inlined from former @a2a-client/embedding package)
 */
...
export class EmbeddingClient {
    ...
    async embed(text: string): Promise<number[]> {
        ...
        switch (this.provider) {
            case PROVIDERS.LOCAL_HUB: embedding = await this._embedLocalHub(text); break;
            case PROVIDERS.OPENAI: embedding = await this._embedOpenAI(text); break;
            case PROVIDERS.COHERE: embedding = await this._embedCohere(text); break;
            case PROVIDERS.VOYAGE: embedding = await this._embedVoyage(text); break;
            case PROVIDERS.MOCK:
            default: embedding = this._embedDeterministic(text);
        }
    ...
    async embedBatch(texts: string[]): Promise<number[][]> {
        ...
        for (const text of texts) {
            if (!text || text.trim().length === 0) { results.push(this._zeroVector()); continue; }
            const cacheKey = this._hashText(text);
            if (this.cache.has(cacheKey)) { results.push(this.cache.get(cacheKey)!); }
            else { results.push(null); toEmbed.push({text, cacheKey}); }
        }

        if (toEmbed.length > 0 && this.provider !== PROVIDERS.MOCK) {
            let embeddings: number[][];
            switch (this.provider) {
                case PROVIDERS.LOCAL_HUB: embeddings = await this._embedBatchLocalHub(toEmbed.map((t) => t.text)); break;
                case PROVIDERS.OPENAI: embeddings = await this._embedBatchOpenAI(toEmbed.map((t) => t.text)); break;
                case PROVIDERS.COHERE: embeddings = await this._embedBatchCohere(toEmbed.map((t) => t.text)); break;
                case PROVIDERS.VOYAGE: embeddings = await this._embedBatchVoyage(toEmbed.map((t) => t.text)); break;
                default: embeddings = toEmbed.map((t) => this._embedDeterministic(t.text));
            }
            for (let i = 0; i < toEmbed.length; i++) {
                const emb = embeddings[i];
                if (emb) { this.cache.set(toEmbed[i].cacheKey, emb); const idx = results.indexOf(null); if (idx !== -1) results[idx] = emb; }
            }
    ...
    private async _embedOpenAI(text: string): Promise<number[]> {
        ...
        return ((await response.json()) as {data: Array<{embedding: number[]}>}).data[0].embedding;
    }
    ...
    private async _embedBatchOpenAI(texts: string[]): Promise<number[][]> {
        ...
        if (!response.ok) throw new Error(`OpenAI API error: ${response.status} ${await response.text()}`);
        return ((await response.json()) as {data: Array<{embedding: number[]}>}).data.map((d) => d.embedding);
    }
    ...
    private async _embedCohere(text: string): Promise<number[]> {
        ...
        return ((await response.json()) as {embeddings: number[][]}).embeddings[0];
    }
    ...
    private async _embedBatchCohere(texts: string[]): Promise<number[][]> {
        ...
        for (let i = 0; i < texts.length; i += 96) {
            ...
            if (!response.ok) throw new Error(`Cohere API error: ${response.status} ${await response.text()}`);
            all.push(...((await response.json()) as {embeddings: number[][]}).embeddings);
        }
        return all;
    }
    ...
    private async _embedBatchVoyage(texts: string[]): Promise<number[][]> {
        ...
        for (let i = 0; i < texts.length; i += 64) {
            ...
            if (!response.ok) throw new Error(`Voyage API error: ${response.status} ${await response.text()}`);
            all.push(...((await response.json()) as {data: Array<{embedding: number[]}>}).data.map((d) => d.embedding));
        }
        return all;
    }
    ...
    getCacheStats(): CacheStats {
        return {size: this.cache.size, provider: this.provider, model: this.model, dimension: this.getDimension(), baseUrl: this.baseUrl};
    }
    ...
    async isAvailable(): Promise<boolean> {
        try {
            switch (this.provider) {
                ...
                case PROVIDERS.OPENAI: return !!(this.apiKey ?? process.env.OPENAI_API_KEY);
                case PROVIDERS.COHERE: return !!(this.apiKey ?? process.env.COHERE_API_KEY);
                case PROVIDERS.VOYAGE: return !!(this.apiKey ?? process.env.VOYAGE_API_KEY);
                default: return true;
            }
        } catch (e) {
            console.error('[EmbeddingClient] isAvailable check failed:', e instanceof Error ? e.message : String(e));
            return false;
        }
    }
    ...
    async listModels(): Promise<string[]> {
        if (this.provider === PROVIDERS.LOCAL_HUB) {
            ...
            if (!response.ok) throw new Error(`[EmbeddingClient] listModels failed: ${response.status} ${await response.text()}`);
            return ((await response.json()) as {models?: Array<{name: string}>}).models?.map((m) => m.name) ?? [];
        }
        return [this.model];
    }
