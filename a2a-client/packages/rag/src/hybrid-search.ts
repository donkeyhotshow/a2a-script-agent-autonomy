...
export interface HybridSearchConfig {
    sparseSearch?: SparseSearchLike | {
        search: (q: string, o: { limit?: number }) => Promise<{
            hits?: Array<{ id?: string; path?: string; content?: string; _rankingScore?: number }>
        }>
    };
...
export class HybridSearcher {
    ...
    private async _searchSparse(query: string, limit: number): Promise<RRFDoc[]> {
        ...
        try {
            ...
            const result = await (this.sparseSearch as {
                search(q: string, o: { limit: number }): Promise<{ hits?: MeiliHit[] }>
            }).search(query, {limit});
