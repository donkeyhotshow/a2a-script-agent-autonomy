...
- **coder** — form (message) → result.message; then execute.rag-search → result.rag-search; execute.read-file →
  result.read-file; execute.write-file → result.write-file; etc.
- **analyze-dialog** — form with choices (continue_search / save_report) → result.choice + optional result.message /
  result.path.
