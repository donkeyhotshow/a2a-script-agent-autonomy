...
In **Actions** mode, the **server controls the workflow** by sending a predefined sequence of steps. The client simply executes each step and returns results.
...
In **AI-Actions** mode, the **LLM controls the workflow**. The server provides tools/capabilities, but the LLM decides which tools to call and in what order.
