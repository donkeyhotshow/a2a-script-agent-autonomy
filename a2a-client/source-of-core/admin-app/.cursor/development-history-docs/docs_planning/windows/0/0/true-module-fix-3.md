...
- **Сценарий не стартует или застрял** — удали marker-файл вида
  `engine/markersrioId>_<TaskId>.progress.json` (или с `default` вместо TaskId, если запускался без
  TaskId).
...
- Изучи, как `[invoke-scenario-engine.php](mdc:script/engine/invoke-scenario-engine.php)` использует `ScenarioId`,
  ...
  прогресса (например, `engine/markersRIO_ID>_<TaskID_или_default>.progress.json`). Эти маркеры
