...
- **Сценарий не стартует или застрял** — удали marker-файл вида
  `engine/markers<TaskId>.progress.json` (или с `default` вместо TaskId, если запускался без
  TaskId).
...
- Изучи, как `[invoke-scenario-engine.php](mdc:script/engine/invoke-scenario-engine.php)` использует `ScenarioId`,
  ...
  прогресса (например, `engine/markers_<TaskID_или_default>.progress.json`). Эти маркеры
