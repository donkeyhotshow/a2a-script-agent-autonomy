...
## Дополнительные документы разработки

Ниже приведены ссылки на документы из папки разработки (@development):

- [json-module-index.md](/docs/development/json-module-index.md) — строки: 1–185
- [lessons-learned.md](/docs/development/lessons-learned.md) — строки: 1–323
- [review-ui.md](/docs/development/review-ui.md) — строки: 1–816
- [StrictModuleChecklist.md](/docs/guides/StrictModuleChecklist.md) — строки: 1–160
- [standards-review-summary.md](/docs/development/standards-review-summary.md) — строки: 1–152
- [module-structure-and-installation.md](/docs/development/module-structure-and-installation.md) — строки: 1–43
- [review-script-system.md](/docs/development/review-script-system.md) — пустой файл
- [review-primary-form-module.md](/docs/development/review-primary-form-module.md) — строки: 1–159
- [review-qtu-module.md](/docs/development/review-qtu-module.md) — строки: 1–159
- [review-render.md](/docs/development/review-render.md) — строки: 1–1633
...
5. **Адаптация Старых Версий к Новым Стандартам**:
    ...
    * Это означает, что если `v5` переименовал общее действие с `process-questions` на `loadQuestions` и изменил его
...
1. **Сканируй** свой модуль и сравнивай с эталонными рабочими модулями (`primary-form/v1`, `login-form/v1`,
   `playground/v1`).
    - **Для AI ассистента**: Придерживайся этого же принципа. "Эталонные" модули (за исключением `question-to-user`,
      `config`, `contact-form`, если не указано иное) являются источником истины для предпочитаемых паттернов, даже если
      они не детализированы в текущем чеклисте.
...
3. **Выпиши ошибки** (по файлам, по типам, по ключам).
    - **Для AI ассистента**: Если пользователь исправляет ошибку в твоей работе, тщательно проанализируй исправление.
      Затем проактивно проверь другие файлы, с которыми ты недавно работал, на наличие аналогичных логических ошибок или
      неверно примененных паттернов, и исправь их.
    ...
    - Для docs — не добавляй туда UI-компоненты.
      **4.a. Проверяй корректность префиксов DataHub**:
        ...
        - **Последствия**: Использование невалидного префикса приведет к ошибке выполнения экшена, так как
          ...
          **4.b. Работа AI ассистента с чеклистами и задачами**:
        - **Основной документ**: `finalization-checklist.md` (или аналогичный чеклист задачи) является главным
          управляющим документом.
        - **Детализация чеклиста**: Общие пункты чеклиста должны быть детализированы AI ассистентом. Например, если
          пункт гласит "Проверить все action-файлы на X", ассистент должен сначала составить список всех action-файлов в
          модуле, а затем систематически проверять каждый файл из списка по критерию X, отмечая прогресс. После
          завершения работы со списком, исходный пункт чеклиста помечается выполненным.
        - **Проактивное продолжение**: После выполнения всех явных инструкций от пользователя и обновления чеклиста, AI
          ассистент должен самостоятельно переходить к следующему невыполненному пункту в чеклисте. Не следует ожидать
          явной команды "что дальше".
        - **Разрешение противоречий и неясностей**: Если инструкция пользователя, правило из `custom_instructions` или
          пункт чеклиста кажутся противоречивыми или неясными, AI ассистент должен явно обозначить это противоречие и
          запросить у пользователя разъяснение. В идеале, для таких запросов используется модуль QTU. Если QTU
          недоступен, можно задать до 5 уточняющих вопросов формата да/нет в конце своего сообщения.
        - **Предложения по улучшению**: Если в ходе работы выявляются возможности для улучшения процесса, стандартов или
          самого чеклиста (например, добавление нового важного пункта проверки), AI ассистент должен предложить это,
          обычно путем добавления соответствующего пункта в чеклист для последующего рассмотрения.
        - **Интерпретация инструкций**: Инструкции пользователя "забудь о X" означают полное прекращение работы с X.
          Сильные эмоциональные маркеры в сообщениях пользователя ("бред", "ебать") являются критическим сигналом к
          немедленной остановке текущего подхода и его переоценке.
          **4.c. Проектирование Actions и UI Страниц**:
        - **Целенаправленный дизайн Action-файлов**: Каждый `actions/*.json` файл должен быть спроектирован для
          выполнения четко определенной логической операции. Часто это связано с обработкой пользовательского ввода (
          например, нажатие кнопки на UI) или выполнением ключевых системных функций (например, генерация данных,
          сохранение состояния, применение изменений, удаление информации). Избегайте создания actions со смешанной или
          неясной ответственностью.
        - **Систематическое проектирование UI Страниц**: Разработка UI страниц (`pages/*.json`) должна включать
          следующие этапы:
            - **Инвентаризация**: Составьте список всех страниц модуля.
            - **Определение назначения**: Для каждой страницы четко сформулируйте ее основное предназначение и какие
              задачи пользователя она решает.
            - **Обработка данных**: Определите, какие данные страница будет отображать и с какими данными пользователь
              будет взаимодействовать.
            - **Интерактивность и Actions**: Спроектируйте, какие кнопки и другие интерактивные элементы будут
              присутствовать на странице, какие actions они будут инициировать, и какие данные будут передаваться в эти
              actions или ожидаться от них.
            - **Внедрение форм**: Для страниц, включающих ввод данных, реализуйте формы, ориентируясь на лучшие практики
              и примеры из эталонных модулей (например, `primary-form/v2`), обращая внимание на структуру данных формы,
              валидацию и механизм отправки.

        - **Справочник по основным Action-ам в `type: "Instructions"`**: (Подробнее см.
          `docs/ui/commands-and-operations/server-actions-reference.md` и `docs/guides/module/06-actions-logic.md`)
          Каждая инструкция — это JSON-объект с обязательным ключом `action`. Общие поля, которые могут присутствовать
          во многих инструкциях:
            - `from` / `address`: (String|Array<String>) DataHub адрес(а) источника данных.
            - `to`: (String) DataHub адрес назначения данных.
            - `value`: (Любой тип) Прямое литеральное значение (можно использовать плейсхолдеры `{...}` для подстановки
              переменных).
            - `condition`: (String|Array) Условие для выполнения инструкции (см. раздел "Нюансы `condition` в actions"
              в "Типовые ошибки и паттерны исправления").
            - `disabled`: (Boolean) Если `true`, инструкция пропускается.

          **Основные типы `action`:**

            1. **`update`**: Модифицирует данные в DataHub.
                * **Назначение**: Основное действие для манипуляции данными (копирование, установка значения, поиск и
                  обновление).
                * **Ключевые параметры**:
                    * `from` / `address`: (Опционально, если есть `value`) Источник данных.
                    * `to`: (Обязательно) Цель для записи.
                    * `value`: (Опционально, если есть `from`) Прямое значение.
                    * `find`: (Объект, опционально) Для поиска внутри данных `from`.
                        * `value`: Значение или `{path}` для поиска.
                        * `return`: (Опционально) Что вернуть из найденного элемента (`key`, `value`, или путь внутри).
                    * `with`: (String|Array, опционально) Имя трансформации/метода для применения к данным (например,
                      `count`, `json_decode`, `stringify`).
                    * `batch`: (Массив, опционально) Массив других инструкций `update` для атомарного пакетного
                      выполнения (каждая может иметь свой `condition`).
                * **Пример**: `{"action": "update", "from": "input:userId", "to": "buffer:currentUserId"}`

            2. **`save`**: Сохраняет данные из DataHub в постоянное хранилище (например, файл, модель).
                * **Назначение**: Запись данных из памяти (DataHub) на диск или в БД.
                * **Ключевые параметры**:
                    * `from`: (String|Array<String>, обязательно) DataHub адрес(а) сохраняемых данных (например,
                      `models/someModel`, `module-name/data/someFile`). **Не может быть `buffer:`**.
                * **Пример**: `{"action": "save", "from": "primary-form/data/user-profile"}`

            3. **`add`**: Добавляет элемент в массив или конкатенирует строки в DataHub.
                * **Назначение**: Модификация коллекций или строк путем добавления в начало/конец.
                * **Ключевые параметры**:
                    * `from` / `address` / `value`: Источник данных для добавления.
                    * `to`: (Обязательно) Адрес массива/строки для модификации.
                    * `how`: (String, опционально) `append` (по умолчанию) или `prepend`.
                    * `limit`: (Число, опционально) Максимальный размер массива после добавления.
                * **Пример**: `{"action": "add", "from": "buffer:newLogEntry", "to": "primary-form/data/activity-log"}`

            4. **`remove`**: Удаляет данные из DataHub и сохраняет изменение (если применимо к постоянному хранилищу).
                * **Назначение**: Удаление данных.
                * **Ключевые параметры**:
                    * `from` / `to`: (String|Array<String>, обязательно) Адрес(а) для удаления.
                * **Пример**: `{"action": "remove", "from": "buffer:tempUserData"}`

            5. **`call`**: Выполняет другую последовательность инструкций из файла.
                * **Назначение**: Модуляризация и переиспользование последовательностей actions.
                * **Ключевые параметры**:
                    * `from` / `address`: (String, обязательно) Адрес JSON-файла с инструкциями (например,
                      `primary-form/commands/reset-form`).
                    * `args`: (Объект, опционально) Аргументы для передачи в вызываемый action (будут доступны в нем
                      через `input:argName`).
                * **Пример**:
                  `{"action": "call", "from": "primary-form/commands/reset-form", "args": {"userId": "{buffer:userId}"}}`

            6. **`for`**: Итерирует по массиву/объекту, выполняя инструкции для каждого элемента.
                * **Назначение**: Циклическая обработка коллекций.
                * **Ключевые параметры**:
                    * `from` / `address`: (String, обязательно) Адрес итерируемой коллекции.
                    * `instructions`: (Массив, обязательно) Инструкции для выполнения на каждой итерации.
                    * `to`: (String, опционально) Адрес для записи массива результатов (если дочерние инструкции пишут в
                      `buffer:for.list`).
                    * (Внутри `instructions` доступны `buffer:for.currentItem`, `buffer:for.currentIndex`,
                      `buffer:for.parentItem` для вложенных циклов).
                * **Пример**:
                  `{"action": "for", "from": "input:items", "instructions": [{"action": "update", "from": "buffer:for.currentItem.name", "to": "buffer:tempNames.{buffer:for.currentIndex}"}]}`

            7. **`return`**: Завершает текущую последовательность инструкций и добавляет данные в финальный ответ для
               клиента.
                * **Назначение**: Отправка данных обратно в UI и завершение action.
                * **Ключевые параметры**:
                    * `from` / `address` / `value`: Источник данных для возврата.
                    * `halt`: (Boolean, опционально) Если `true`, останавливает дальнейшую обработку запроса.
                * **Пример**: `{"action": "return", "value": {"status": "success", "message": "Done"}}`

            8. **`print_r`**: Отладочное действие для вывода значения переменной в историю выполнения шага.
                * **Назначение**: Инспектирование данных во время выполнения.
                * **Ключевые параметры**:
                    * `from` / `address` / `value`: Источник данных для вывода.
                * **Пример**: `{"action": "print_r", "from": "buffer:myVariable"}`

            9. **`comment`**: Добавляет комментарий в трассировку выполнения.
                * **Назначение**: Добавление описательных комментариев в поток actions для удобочитаемости/отладки.
                * **Ключевые параметры**: `message` или `text` (Строка).
                * **Пример**: `{"action": "comment", "message": "Этап валидации пользователя"}`

          **Важно**: Всегда сверяйтесь с актуальной документацией (
          `docs/ui/commands-and-operations/server-actions-reference.md`, `docs/guides/module/06-actions-logic.md`) и
          рабочими примерами в эталонных модулях для наиболее точной информации по параметрам и поведению.

5. **После каждого исправления — снова запускай merge и validate**.
    - **Для AI ассистента**: При вызове инструментов, таких как `edit_file`, если результат (diff) не соответствует
      ожиданиям, всегда следует сначала попытаться использовать `reapply` перед тем, как пробовать альтернативное
      редактирование.
6. **Фиксируй опыт**: после успешного исправления — добавь уроки в `lessons-learned.md`, обнови changelog, при
   необходимости — чеклисты.
    - **Для AI ассистента**: Отмечай выполненные пункты в чеклисте. Объяснения действий должны быть на уровне "для
      профессионала", без извинений. Указывай на не-абстрактные ошибки в промптах пользователя. Используй обратные
      кавычки (`) для путей к файлам и ключей JSON для точности.
...
- **Проблемы с инструментами AI-ассистента (Tooling Caveats)**:
    - **`read_file` и большие файлы**: Инструмент `read_file` может критически обрезать большие JSON файлы (например,
      `actions/process-questions.json`), даже если запрошено чтение всего файла. Это серьезно затрудняет анализ и
      рефакторинг. Старайтесь работать с меньшими, более сфокусированными файлами, если это возможно, или будьте готовы
      к ручной проверке полноты данных.
    - **`edit_file`**: Иногда может некорректно применять изменения, особенно при полной замене содержимого или
      множественных правках в больших файлах. Всегда проверяйте diff после применения. Используйте `reapply`, если
      результат не соответствует ожиданиям, прежде чем пытаться внести правку другим способом.
    - **Несоответствия валидатора**: Валидатор UI (`php artisan validate:module-json`) может показывать ошибки для
      корректных свойств некоторых компонентов (например, `options`, `optionLabel`, `optionValue` для компонента
      `Select`). В таких случаях сверяйтесь с `install-modules/aiCore/js/component-map.json` и примерами из эталонных
      модулей. Если компонент и его свойства используются в рабочих эталонных модулях, возможно, проблема в схеме
      валидатора, а не в вашем коде. Отметьте это в `known-issues.md` модуля.
...
- **Файл финализации модуля**: Сверяйтесь с `finalization-checklist.md` самого исправляемого модуля (если он существует,
  например, `implement-modules/question-to-user/v5/docs/finalization-checklist.md`), так как он может содержать наиболее
  актуальные специфические для модуля проблемы, решения и последние выводы по синтаксису и структуре.
...
- Исследуй не только структуру, но и связи между actions, сценариями, task_definitions, task_types, markers,
  state-файлами.
    - **Для эффективного сканирования связей и поиска релевантных файлов, активно
      ...
      упоминания модуля в коде и документации.** (См. `[AI Task System Script Workflow](mdc:script-workflow)`
      и `[main-index.ps1](mdc:main-index.ps1)).
...
- **Пример из практики (`question-to-user/v5`)**: В ходе рефакторинга `question-to-user` до версии `v5`, важным этапом
  сканирования было определение текущего состояния его компонентов (например, `actions/process-questions.json`,
...
- **Нюансы `condition` в actions**:
    - **Расхождение валидатора и PHP**: Валидатор JSON (`validate:module-json`) может пропускать строковые `condition`
      вида `"{buffer:valA && buffer:valB}"` из-за гибкого regex. Однако, PHP-движок (
      `DataManipulateHelper::evaluateCondition`) интерпретирует строки только как прямые DataHub пути (например,
      `"buffer:myFlag"` или `"!buffer:myFlag"`) и **не выполняет** сложную логику (`&&`, `||`) из таких строк. Любая
      невалидная для PHP строка условия приведет к ошибке выполнения или некорректной логике.
    - **Массивы для сложной логики**: Любая сложная логика в `condition` (И, ИЛИ, сравнения и т.д.) **обязательно**
      должна использовать синтаксис массива, например, `["and", "buffer:pathA", "buffer:pathB"]`. Только так PHP-движок
      корректно обработает условие.
    - **Ограничения `DataManipulateHelper`**: Помните, что текущая реализация `DataManipulateHelper.php` может не
      поддерживать все желаемые логические операторы в формате массива (например, `OR` или `INCLUDES` для массивов). В
      таких случаях может потребоваться доработка PHP-хелпера или изменение логики JSON (например, через промежуточные
      буферные переменные) для достижения нужного результата с помощью существующих операторов.
- **Использование `batch`**: Ключ `batch` является параметром внутри других actions (например, `action: "update"`,
  `"batch": [...]`), позволяя группировать операции. Это **не** самостоятельный `action: "batch"`. Хотя это валидный
  способ группировки, в некоторых случаях (например, для упрощения отладки, обхода сложностей с валидатором или если
  вложенные условия становятся слишком комплексными) может быть предпочтительнее "развернуть" такие пакетные операции в
  последовательность отдельных инструкций.
