...
## Рендеринг вложений (attachments)
...
### Обработка в `Render.renderExecute()`:

1. **RAG Search**: Если есть `attachments.ragQuery`, UI показывает индикатор "Поиск в кодовой базе: [query]".
2. **Файлы**:
   - `readFiles`: Список прочитанных файлов.
   - `writtenFiles`: Список измененных файлов.
3. **Команды**: `shellCommand` отображает выполняемую в данный момент терминальную команду.
4. **Статус**: Если `message` отсутствует, UI генерирует его на основе `pendingClientAction` (например, "Выполнение скрипта…").

> [!NOTE]
> Вложения предназначены только для **отображения состояния**. Сами данные (содержимое файлов, результаты поиска) остаются в `context` сессии и не должны пробрасываться в упрощенный `execute` блок для UI.
