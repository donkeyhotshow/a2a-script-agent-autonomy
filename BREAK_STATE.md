...
| **3 — Drift found** | `prepareServerRequest` only ran `sanitizeContextForServer` on `context`; top-level body keys were not passed through `sanitizeInvokeBodyForA2aUpstream` (SDK already did). Low risk today (body shape is `{ context, result, task?, sync? }`) but **not parity** if top-level client keys are added later. **Fixed:** return `sanitizeInvokeBodyForA2aUpstream(requestToServer)` from `prepareServerRequest`. |
