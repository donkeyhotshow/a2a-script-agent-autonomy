...
class ErrorClassifier {
  constructor() {
    this.errorPatterns = [
      ...
      { pattern: /local_hub|model.*not.*found|no.*such.*model/i, type: 'llm', subtype: 'model',
        hint: 'Proxy model issue. Check: curl http://localhost:11434/api/tags and ensure the model is available via proxy.',
        severity: 'critical' },
      ...
      { pattern: /promise.*stuck|processing.*forever/i, type: 'task', subtype: 'promise-stuck',
        hint: 'Promise stuck in processing. Check proxy status with curl http://localhost:11434/api/ps',
        severity: 'high' },
    ...
    this.directTests = {
      ...
      localLlmTags: 'curl http://localhost:11434/api/tags',
      localLlmPs: 'curl http://localhost:11434/api/ps',
  ...
  suggestQuickFix(classification) {
    const fixes = {
      ...
      'http:unavailable': 'Check proxy status: curl http://localhost:11434/api/ps',
      ...
      'task:timeout': 'Check proxy generation: curl http://localhost:11434/api/ps',
      ...
      'async:infinite-pending': 'Check proxy status: curl http://localhost:11434/api/ps',
