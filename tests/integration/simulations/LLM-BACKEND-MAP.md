...
Goldens in `simulations/` exercise the **A2A server + transforms + Client API** contract. They do **not** call Z.AI or Local LLM upstream unless you run a live stack. Use this map to know **what is fixture-only** vs **what the real proxy returns**.

## Runtime stack (live)

| Layer | Port (typical dev) | Role |
|-------|-------------------|------|
| **Local LLM upstream** | 11435 | Local models (e.g. `qwen3:8b`) |
...
**Source of model list at runtime:** `http://localhost:11434/api/tags` (proxy), not raw Local LLM upstream alone.
...
---
llm_assumed:
  provider: compat_llm   # z_ai | compat_llm | synthetic
  model: qwen3:8b
...
2. Add a small sync golden that shows one step with **Z.AI-shaped** and one with **Local LLM upstream-shaped** upstream (or document both in one `description.md` table).
