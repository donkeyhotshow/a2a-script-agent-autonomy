...
cd ai-integration
start /b "" cmd /c "set OLLAMA_HOST=http://localhost:%OLLAMA_PORT% && set OLLAMA_MODELS=%OLLAMA_MODELS% && python -m uvicorn proxy.asgi:application --host 0.0.0.0 --port %PROXY_PORT%"
cd ..
