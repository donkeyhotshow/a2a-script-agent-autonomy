...
- **A2A Server → AI Hub:** `AI_HUB_URL` (default `http://localhost:11434`) in `request-processor.service.ts`.
- **Embedding Ollama:** `OLLAMA_BASE_URL` default `http://localhost:11435` in `@a2a/embedding`.
...
|----|---------|---------|-----------|
| `doc-defaults` | Match `AGENTS.md` table | Server 3000, Web 5173, AI Hub 11434, Ollama 11435. | Fix `.env.example` if it disagrees (known drift risk). |
...
- **Cost / risk:** confusion if legacy `.env` swaps 11435/11434 meanings.
- **Status:** candidate
...
| Ollama (generate) | http://localhost:11435 |
| Ollama (embeddings if local) | http://localhost:11435 |
