...
| suggest-rest-conventions | api-design | compat_llm | Предложение REST conventions | all | ✅ |
| suggest-graphql | api-design | compat_llm | Предложение GraphQL | all | ✅ |
...
| suggest-eager-loading | database | compat_llm | Предложение eager loading | all | ✅ |
...
| suggest-query-optimization | database | compat_llm | Предложение оптимизации запросов | all | ✅ |
| detect-slow-queries | database | script | Детекция медленных запросов | all | ✅ |
| suggest-caching-strategy | caching | compat_llm | Предложение стратегии кэширования | all | ✅ |
...
| suggest-cdn-integration | caching | compat_llm | Предложение CDN | all | ✅ |
| detect-queue-usage | queues | script | Детекция использования очередей | all | ✅ |
| suggest-message-queue | queues | compat_llm | Предложение message queue | all | ✅ |
...
| suggest-service-mesh | architecture | compat_llm | Предложение service mesh | all | ✅ |
| detect-monolith-issues | architecture | script | Детекция проблем монолита | all | ✅ |
| suggest-domain-driven-design | architecture | compat_llm | Предложение DDD | all | ✅ |
| detect-middleware | middleware | script | Детекция middleware | all | ✅ |
| suggest-rate-limiting | middleware | compat_llm | Предложение rate limiting | all | ✅ |
...
| suggest-jwt | auth | compat_llm | Предложение JWT | all | ✅ |
| suggest-oauth | auth | compat_llm | Предложение OAuth | all | ✅ |
| implement-2fa | auth | agent | Реализация 2FA | all | ⏳ |
| detect-validation | validation | script | Детекция валидации | all | ✅ |
| suggest-validation-library | validation | compat_llm | Предложение библиотеки валидации | all | ✅ |
| detect-error-handling | error-handling | script | Детекция обработки ошибок | all | ✅ |
| suggest-error-tracking | error-handling | compat_llm | Предложение error tracking | all | ✅ |
...
| suggest-structured-logging | logging | compat_llm | Предложение structured logging | all | ✅ |
| implement-elk-stack | logging | agent | Реализация ELK stack | all | ⏳ |
| detect-websockets | realtime | script | Детекция WebSockets | all | ❌ |
| suggest-socket-io | realtime | compat_llm | Предложение Socket.io | all | ❌ |
...
| suggest-file-storage | files | compat_llm | Предложение хранилища файлов | all | ✅ |
