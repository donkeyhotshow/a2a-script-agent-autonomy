...
Документи в `docs/new-request-flow/` описують ідеальну архітектуру:
...
| `server-transforms-request.json`  | Server (опціонально) | Трансформація запиту перед відправкою до LLM      |
| `request.md`                    | Server → LLM         | **MARKDOWN** з system prompt!                     |
| `response.md`                   | LLM → Server         | Відповідь від LLM                                 |
| `server-transforms-response.json` | Server (опціонально) | Трансформація відповіді перед поверненням клієнту |
...
cd ai-integration && python app/main.py
```
