...
Every mode has a curated list of JSON blobs inside `docs/production/test-requests/`:
...
- `ollama.json` — raw Ollama payloads (`model` + `prompt`) that land at `POST /api/generate` on direct Ollama (port 11434).
...
- `--client-url`, `--server-url`, `--ollama-url` — override the base URLs used for each mode (defaults: `http://localhost:3001`, `http://localhost:3000`, `http://localhost:11434`).
...
- Ensure the stack is running (`npm run dev` or equivalent) before invoking the script. Client/server/ollama health checks are described in `docs/production/SYSTEM_SCHEMA.md`.
...
- **Auth**: None required
- **Note**: Direct Ollama runs on port 11434. Use ai-integration proxy port 11435 only if it's properly configured to connect to Ollama.
...
**404 on `/api/generate`**
- Make sure to use port 11434 (direct Ollama) or 11435 (ai-integration proxy) correctly.
- Check if Ollama is running: `curl http://localhost:11434/api/tags`

**Model not found (`model 'qwen3:8b' not found`)**
- Ensure the model is available in Ollama: `curl http://localhost:11434/api/tags`
- If using ai-integration proxy on 11435, check OLLAMA_HOST configuration

**Empty models list**
- For direct Ollama (11434): Check if Ollama is running and models are loaded
- For ai-integration proxy (11435): Check OLLAMA_HOST in config - it should point to actual Ollama port (11434)

### Port Configuration Issue

**Current Setup:**
- Direct Ollama: Port 11434 (PID 29572 - python3.13.exe - ai-integration crashed)
- AI Integration Proxy: Port 11435 (PID 45752 - ollama.exe - direct Ollama started on 11435 by mistake)

This is wrong! Ollama should be on 11434, and ai-integration proxy should connect to it.

**Fix:**
1. Stop all services
2. Make sure OLLAMA_HOST in ai-integration/config is set to `http://localhost:11434`
...
| AI Integration proxy | 11435 | `GET http://localhost:11435/` (returns empty if not Flask) |
| Ollama (direct) | 11434 | `GET http://localhost:11434/api/tags` |
