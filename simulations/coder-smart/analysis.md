...
1. **Діалог + RAG (3–6):** форма (запит) → RAG clarify → LLM clarify → RAG plan → LLM plan → LLM checklist → write-file .carrier/tasks/.
2. **Цикл (7–9+):** form (контент док) → LLM execute item → write-file оновленого док → history обнуляється → form знову → наступний пункт.
