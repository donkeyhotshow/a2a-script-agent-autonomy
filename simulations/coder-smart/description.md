...
├── 4/ request.json, request.md, response.md, response.json  # RAG results → LLM clarify → doc 1+2, execute.rag-search (plan)
├── 5/ request.json, request.md, response.md, response.json  # RAG results → LLM research plan → doc 1+2+3
├── 6/ request.json, request.md, response.md, response.json  # LLM checklist → doc 1+2+3(done)+4, execute.write-file
├── 7/ request.json, response.json   # result.written → execute.form (doc content)
├── 8/ request.json, request.md, response.md, response.json   # result.message (doc) → LLM execute item → write-file
