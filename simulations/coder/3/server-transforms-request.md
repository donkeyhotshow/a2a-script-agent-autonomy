...
**Input:** `request.json` = `{ "context": { "task": "допоможи розібратись з кодом", "execution": { "action": "coder-dialog" } }, "result": { "message": "як працює система авторизації?" } }`.
...
3. Create `request.md`:
   - System prompt from coder-dialog action definition
