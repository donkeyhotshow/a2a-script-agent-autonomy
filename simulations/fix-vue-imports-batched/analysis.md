...
2. **search-exporter** (internal) → executes `rag-search` на клієнті (в циклі для кожного файлу). Це осмислене
   використання `rag-search` для пошуку експортів у кодовій базі.
