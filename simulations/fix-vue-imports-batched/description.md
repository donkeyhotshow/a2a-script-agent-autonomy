...
Ця симуляція — **конкретний приклад** послідовної багатокрокової обробки з накопиченням даних у контексті. Той самий патерн
...
- **Пакетна обробка файлів** — тут одиниці циклу — Vue-файли; інші сценарії можуть “пакувати” сторінки RAG, пункти плану тощо.
...
- **Немає LLM на рішеннях переходів** — логіка переходів детермінована; але **інші** потоки можуть поєднувати той самий накопичувальний контекст з LLM.

Додатково дивіться таблицю **Planned batch-style flows (roadmap)** у тому ж [`../SCHEMA.md`](../SCHEMA.md) – вона
перелічує інші командні/послідовні потоки, які можна моделювати аналогічним способом: RAG-пагінація, черга `read-file`, chunked scans тощо.
