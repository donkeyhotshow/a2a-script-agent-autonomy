...
Sister folder next to [`../6/`](../6/) (name = parent step + sequential index). **Server-internal** golden only (
[`request.json`](./request.json) → [`response.json`](./response.json)); no Web `client.json` / `received.json`. Documents *
