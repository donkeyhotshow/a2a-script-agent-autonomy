...
2. Match actions for this task. No-LLM actions (e.g. fix-vue-imports) have higher priority; fallback (auto-ai,
...
3. Build response: `context` = `{ task }`, `execute.form` with `choices`: no-LLM actions first, then fallbackActions
   merged (fix-vue-imports → auto-ai → task-decomposition).
