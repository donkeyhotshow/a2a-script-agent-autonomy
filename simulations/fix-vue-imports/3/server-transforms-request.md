...
**Input:** `request.json` =
`{ "context": { ..., "execution": { "action": "fix-vue-imports", "step": "vue-import-detect" } }, "result": { "broken_imports": [...] } }`.

1. Take `context` from request.
2. From current `execution.step` (vue-import-detect) and `result.broken_imports`, advance to `execution.step` =
   `"vue-import-resolve"`.
