# Auto-AI v2 — hand-authored golden (ISSUE 6 / ISSUE 9)
...
1. **Router** — `task` / `new` → `execute.form.choices` includes `auto-ai-v2`.
2. **Choice** — `result.choice: auto-ai-v2` → `execute.form.input` (message).
...
```bash
cd a2a-server && npx tsx scripts/regen-auto-ai-v2-request-md.ts
```
