...
|-------------------|----------|--------|
| **auto-ai-v2 / 6** (this) | `compress_history` | Long tool + assistant history. |
| **auto-ai-v2 / 3–5** | `thinking` or `compress_history` | Earlier turns; use `thinking` if you want `workbench.slots.thinking` before the main reply. |
| **auto-ai-v2 / 3+** (RAG) | `auto_rag_page` | Only if the server implements RAG fetch inside the loop; current stub mainly marks context and re-enters the main LLM. |
